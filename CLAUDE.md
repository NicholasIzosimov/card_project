...
Open `Weightless Deck.html`. No server, no build step. `sim.js` must sit next
to it. `?seed=12345` replays a table exactly.
...
| file | what it is |
|---|---|
| `sim.js` | the solver. DOM-free, headless-testable. **Source of truth.** |
| `Weightless Deck.html` | the page: styles, markup, render + input + UI |
| `deck.body.html` | **generated** — never edit by hand |
